...
thread: $(shell ${THREAD_ID} | xargs -I '{ARG}' printf '${THREAD_DIR}/%s\n' 'html/{ARG}.html' 'csv/{ARG}.csv' 'rdf/{ARG}.rdf')
...
${XSL_DIR}/novelrdf.xsl: $(shell ${THREAD_ID} | xargs printf '${THREAD_DIR}/rdf/%s.rdf\n')

${THREAD_DIR}/rdf/%.rdf: ${THREAD_DIR}/csv/%.csv bin/csv2sioc.sh
	awk -F '\t' -v id=$* -v dep=$< -- '$$1 == id { system(sprintf("bin/csv2sioc.sh \"%s\" \"%s\" \"%s\" \"%s\"", dep, $$4, $$2, $$3)); }' ${THREAD_FILE} >$@

${THREAD_DIR}/csv/%.csv: ${THREAD_DIR}/html/%.html bin/thread2csv.sh
	bin/thread2csv.sh $< >$@

${THREAD_DIR}/html/%.html:
